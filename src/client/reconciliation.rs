...
use crate::net::{CombatStateMessage, PlayerPosition, PlayerSnapshotMessage};
...
///
/// Works on [`PlayerPosition`] (world space) rather than [`Transform`]
/// (screen space) so the comparison is meaningful.
fn reconcile_position(
    ...
    let Ok(mut receiver) = receiver_query.single_mut() else { return };
    ...
    if let Some(snap) = latest {
        ...
        let drift = (server_pos - player_pos.0).length();
...
fn reconcile_combat_state(
    ...
    let Ok(mut receiver) = receiver_query.single_mut() else { return };
    let Ok((mut combat, mut stats)) = player_query.single_mut() else { return };
